...
export default async ({github, context, log = console.log}) => {
  ...
  // We don't want to check PRs from automated tools since they have their own
  // logic for setting titles and labels.
  if (issue.getUser().isBot()) {
    return 0;
  }
  ...
  // Return a process status code that signals error in case there was feedback
  // for the PR. This causes the check to fail and blocks the PR from merging.
  return feedback === "" ? 0 : 1;
}
...
function isValidTitle(title) {
  ...
  // "IOSAP-NNN: Some mandatory text".
  // This is an IATA specific prefix which lives in a different Jira instance.
  if (/^IOSAP-\d+: /.test(title) && !/^IOSAP-\d+:$/.test(title.trim()) && !title.endsWith(".")) {
    return true;
  }

  // "ISAGOAM-NNN: Some mandatory text".
  // This is an IATA specific prefix which lives in a different Jira instance.
  if (/^ISAGOAM-\d+: /.test(title) && !/^ISAGOAM-\d+:$/.test(title.trim()) && !title.endsWith(".")) {
    return true;
  }

  // "ICSO-NNN: Some mandatory text".
  // This is an IATA specific prefix which lives in a different Jira instance.
  if (/^ICSO-\d+: /.test(title) && !/^ICSO-\d+:$/.test(title.trim()) && !title.endsWith(".")) {
    return true;
  }
